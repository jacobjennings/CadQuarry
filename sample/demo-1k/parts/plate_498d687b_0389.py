...
# Generated by CadQuarry v0.1.0 — seed 1234004091 — CC0-1.0
...
PARAMS = {
    ...
    "plate_d": {"type": 'float', "default": 21.4, "min": 8.0, "max": 120.0, "step": 1.0, "group": 'Body', "label": 'Plate depth (mm)'},
    "thickness": {"type": 'float', "default": 10.0, "min": 2.0, "max": 12.0, "step": 0.5, "group": 'Body', "label": 'Thickness (mm)'},
    "hole_d": {"type": 'float', "default": 2.2, "min": 2.0, "max": 2.6, "step": 0.1, "group": 'Holes', "label": 'Hole diameter (mm)'},
...
def build(p):
    ...
    result = (
        ...
        .rarray(22.7, 8.6, p["hole_nx"], p["hole_ny"])
        .hole(p["hole_d"])
    )
    if p["filleted"]:
        result = result.edges('|Z').fillet(min(p["plate_w"], p["plate_d"]) * 0.0719527731749321)
    return result
