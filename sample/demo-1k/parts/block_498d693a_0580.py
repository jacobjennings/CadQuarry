...
# Generated by CadQuarry v0.3.0 — seed 1234004282 — CC0-1.0
...
def build(p):
    ...
    if p["filleted"]:
        result = result.edges('|Z').fillet(min(p["block_w"], p["block_d"]) * 0.09646521188715201)
    if p["filleted2"]:
        result = result.edges('|Z').fillet(min(p["block_w"], p["block_d"]) * 0.1043090829685458)
    return result
