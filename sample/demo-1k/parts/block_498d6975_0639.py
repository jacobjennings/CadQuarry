...
# Generated by CadQuarry v0.3.0 — seed 1234004341 — CC0-1.0
...
PARAMS = {
    ...
    "block_h": {"type": 'float', "default": 63.9, "min": 4.5, "max": 200.0, "step": 1.0, "group": 'Body', "label": 'Block height (mm)'},
...
def build(p):
    ...
    if p["filleted"]:
        result = result.edges('|Z').fillet(min(p["block_w"], p["block_d"]) * 0.04157988925904136)
    if p["filleted2"]:
        result = result.edges('|Z').fillet(min(p["block_w"], p["block_d"]) * 0.11863039046766319)
    return result
