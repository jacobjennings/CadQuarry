...
# Generated by CadQuarry v0.1.0 — seed 1234004738 — CC0-1.0
...
PARAMS = {
    ...
    "plate_d": {"type": 'float', "default": 48.9, "min": 8.0, "max": 120.0, "step": 1.0, "group": 'Body', "label": 'Plate depth (mm)'},
    "thickness": {"type": 'float', "default": 12.0, "min": 2.0, "max": 12.0, "step": 0.5, "group": 'Body', "label": 'Thickness (mm)'},
    "hole_d": {"type": 'float', "default": 6.4, "min": 2.0, "max": 11.0, "step": 0.1, "group": 'Holes', "label": 'Hole diameter (mm)'},
    ...
    "boss_h": {"type": 'float', "default": 10.6, "min": 4.2, "max": 26.5, "step": 1.0, "group": 'Boss', "label": 'Boss height (mm)'},
}
...
def build(p):
    ...
    if p["filleted"]:
        result = result.edges('|Z').fillet(min(p["plate_w"], p["plate_d"]) * 0.11871373194388202)
    result = (
        result.faces('>Z').workplane()
