...
# Generated by CadQuarry v0.1.0 — seed 1234003748 — CC0-1.0
# https://github.com/jacobjennings/CadQuarry

PARAMS = {
    "plate_w": {"type": 'float', "default": 80.14, "min": 20.0, "max": 120.0, "step": 1.0, "group": 'Body', "label": 'Plate width (mm)'},
    "plate_d": {"type": 'float', "default": 61.1, "min": 8.0, "max": 120.0, "step": 1.0, "group": 'Body', "label": 'Plate depth (mm)'},
    ...
    "chamfer_d": {"type": 'float', "default": 2.3, "min": 0.5, "max": 3.0, "step": 0.5, "group": 'Body', "label": 'Chamfer distance (mm)'},
...
def build(p):
    ...
    if p["filleted"]:
        result = result.edges('|Z').fillet(min(p["plate_w"], p["plate_d"]) * 0.05823768084481653)
    if p["chamfered"]:
