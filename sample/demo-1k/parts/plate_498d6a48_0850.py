...
# Generated by CadQuarry v0.1.0 — seed 1234004552 — CC0-1.0
...
PARAMS = {
    ...
    "pocket_w": {"type": 'float', "default": 24.0, "min": 12.0, "max": 36.0, "step": 1.0, "group": 'Pocket', "label": 'Pocket width (mm)'},
    "pocket_d": {"type": 'float', "default": 17.7, "min": 8.8, "max": 27.3, "step": 1.0, "group": 'Pocket', "label": 'Pocket depth (mm)'},
    "pocket_h": {"type": 'float', "default": 4.7, "min": 1.9, "max": 8.5, "step": 0.5, "group": 'Pocket', "label": 'Pocket height (mm)'},
}
...
def build(p):
    ...
    result = (
        ...
        .rarray(36.0, 27.3, p["hole_nx"], p["hole_ny"])
        .hole(p["hole_d"])
    )
    if p["filleted"]:
        result = result.edges('|Z').fillet(min(p["plate_w"], p["plate_d"]) * 0.09056137026244795)
    result = (
        result.faces('>Z').workplane()
