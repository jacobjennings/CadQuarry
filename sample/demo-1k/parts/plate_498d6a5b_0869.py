...
# Generated by CadQuarry v0.3.0 — seed 1234004571 — CC0-1.0
# https://github.com/jacobjennings/CadQuarry

PARAMS = {
    "plate_w": {"type": 'float', "default": 65.5, "min": 20.0, "max": 120.0, "step": 1.0, "group": 'Body', "label": 'Plate width (mm)'},
    ...
    "bolt_r": {"type": 'float', "default": 19.6, "min": 13.1, "max": 29.5, "step": 1.0, "group": 'Holes', "label": 'Bolt circle radius (mm)'},
    ...
    "chamfer_d": {"type": 'float', "default": 1.6, "min": 0.5, "max": 3.0, "step": 0.5, "group": 'Body', "label": 'Chamfer distance (mm)'},
...
def build(p):
    ...
    if p["filleted"]:
        result = result.edges('|Z').fillet(min(p["plate_w"], p["plate_d"]) * 0.10045300884496633)
    if p["chamfered"]:
