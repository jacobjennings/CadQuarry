...
# Generated by CadQuarry v0.6.0 — seed 1234004592 — CC0-1.0
...
PARAMS = {
    ...
    "pocket_d": {"type": 'float', "default": 9.9, "min": 5.0, "max": 14.9, "step": 1.0, "group": 'Pocket', "label": 'Pocket depth (mm)'},
    "pocket_h": {"type": 'float', "default": 1.6, "min": 0.6, "max": 3.4, "step": 0.5, "group": 'Pocket', "label": 'Pocket height (mm)'},
}
...
def build(p):
    ...
    if p["filleted"]:
        result = result.edges('|Z').fillet(min(p["plate_w"], p["plate_d"]) * 0.11254840965030186)
    result = (
        result.faces('>Z').workplane()
