...
\newtcolorbox{keybox}[1][]{
  ...
  fonttitle=\sffamily\bfseries\color{accentd}, title={#1} }
\newtcolorbox{warnbox}[1][]{
  enhanced, breakable, colback=panel2, colframe=accent2,
  boxrule=0pt, leftrule=2.5pt, arc=2pt, left=10pt, right=10pt, top=7pt, bottom=7pt,
  fonttitle=\sffamily\bfseries\color{accent2}, title={#1} }
...
\begin{figure}[h]
  ...
  \caption{A 48-part sample drawn across CadQuarry's eleven part families. Every
...
vocabulary spans plates, brackets, shafts, blocks, multi-section assemblies, and
two solver-built mechanical families---involute/cycloid \emph{gears} and
standards-based \emph{threads} (ISO/ACME/trapezoidal)---raising the complexity
ceiling beyond sketch-extrude primitives. Validity is guaranteed by execution
...
binary STEP, STL, and eight-view GPU renders---is reproducible bit-for-bit from a
versioned seed list, and is browsable through a committed in-browser gallery.
Generation sustains \mbox{$\sim$10} validated parts/s on a 24-core workstation,
paced by the solver-built mechanical families.
We position CadQuarry relative to existing CAD datasets, document the pipeline as
a datasheet, and disclose our use of AI coding assistants in building the tool.
\end{tcolorbox}
...
human-authored Fusion~360 Gallery~\cite{fusion360}, and even the procedurally
generated CAD-Recode~\cite{cadrecode}, are released for \emph{non-commercial} use
(CC~BY-NC variants). Unrestricted---especially commercial---reuse is therefore a
...
\begin{enumerate}[label=\textbf{\arabic*.}]
  ...
  \item A \textbf{raised complexity ceiling}: multi-section assemblies and two
        solver-built mechanical families---involute/cycloid gears and
        standards-based threads---bridged into a single CadQuery solid.
  ...
  \item \textbf{Reproducibility and throughput}: seed $+$ versioned generator
        $\Rightarrow$ bit-identical corpus, produced by a persistent worker pool
        at $\sim$10 validated parts/s (paced by the solver-built mechanical
        families).
  ...
  \item A \textbf{transparent disclosure} of generative-AI usage in the project
        and this manuscript (\cref{sec:ai}).
\end{enumerate}
\end{keybox}
...
CADmium~\cite{cadmium} motivates the geometric-quality metrics we report.
...
bit-for-bit identical regardless of how many workers run. Text artifacts (code,
...
Eleven families span a deliberately broad operation vocabulary
(\cref{fig:families}). Nine are self-contained on CadQuery alone; the two
...
structures, shelled \fm{enclosure}s, \fm{profiled} bars, involute \fm{gear}s,
and standards-based \fm{threaded} parts.}
\label{fig:families}
\end{figure}
...
research, parameter inference, and on-the-fly augmentation. A sidecar
...
# CadQuarry v0.5.0 - seed 1234003889 - CC0-1.0
...
% ===========================================================================
\section{Validity and Deduplication}\label{sec:valid}
% ===========================================================================
...
\fm{threaded} family is sampled at 6\% but realizes 2.8\% of the corpus, because
the finite ISO/ACME standards space saturates and later collisions are dropped.
\end{keybox}

\subsection{Mechanical families: gears and threads}\label{sec:mech}
The \fm{gear} and \fm{threaded} families build real, standards-based geometry
through the build123d~\cite{build123d} stack and bridge it back into CadQuery via
each object's shared OCP \code{.wrapped} handle---so after one line a generated
gear or thread is an ordinary single-solid \code{cq.Workplane} that flows through
the same execution, dedup, export, and customizer paths as every other family.
...
\begin{warnbox}[Reproducibility nuance for solver-built families]
Geometry \emph{signatures} for gears and threads are stable to the dedup
tolerance, so seeded \emph{selection} and dedup hold. Exact STEP/STL \emph{bytes},
however, come from a numeric NURBS/thread solver and are not guaranteed
bit-identical across platforms or library versions the way the primitive families
are. The \code{mech}-stack versions are pinned in the seed list for byte-level
reproduction. Consumers who only read the \code{source} text need nothing extra;
only \emph{re-executing} these parts needs the \code{mech} dependencies.
\end{warnbox}
...
% ===========================================================================
\section{The Corpus: Distribution and Formats}\label{sec:corpus}
% ===========================================================================
...
\textbf{Distribution.} \Cref{fig:dist} shows the realized family mix; complexity
...
\textbf{Formats and the size ladder.} The corpus is published on the Hugging Face
...
% ===========================================================================
\section{Reproducibility and Performance}\label{sec:perf}
% ===========================================================================
\textbf{Reproducibility.} A versioned seed list pins each published corpus to a
tuple of \code{seed}, \code{count}, \code{config}, and generator version.
Re-running the matching
generator reproduces the \emph{same} \code{manifest.jsonl} and the same geometry
signatures bit-for-bit. The published Parquet/JSONL is therefore a
\emph{convenience artifact}: the generator plus the seed list is the canonical
source, and the dataset can always be regenerated from scratch. A geometry
signature is itself versioned---changing its definition would change which parts
count as duplicates---so the dedup rule is part of the reproducibility contract.

\textbf{Performance.} Generation validates every part by execution. Importing
...
paced by the two solver-built mechanical families (\fm{gear} and \fm{threaded},
$\sim$12\% of the mix), which solve real involute/thread geometry at a
meaningful fraction of a second per part; the primitive families alone build in
milliseconds. \Cref{tab:perf} reports wall-clock times for execution-validated
generation (no geometry export); throughput settles at $\sim$10 validated
parts/s and climbs slightly with corpus size as the worker warm-up amortizes.
...
execution-validated and geometry export excluded. Throughput is paced by the
solver-built mechanical families; the optimal worker count is system-dependent
and memory-bound at the largest sizes, so on machines with less RAM fewer
workers than cores may be needed to avoid swapping.}
\label{tab:perf}
\end{table}
...
% ===========================================================================
\begin{warnbox}[Disclosure of AI usage]
...
assistants (Anthropic's Claude and the Cursor agent), under the author's
direction and review; this is recorded in the project's commit history. All
...
(à la CAD-Recode~\cite{cadrecode}); parameter inference and customizable-part
research that exploits the \code{build(p)} interface; and geometry-quality
studies using the structural metrics that recent work emphasizes~\cite{cadmium}.
...
includes real gears and threads, an unambiguous license, and a reproducible,
