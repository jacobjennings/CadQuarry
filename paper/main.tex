...
signature; across a 200{,}000-part generation all 200{,}000 accepted parts are
geometrically distinct. The data is published on the Hugging Face Hub as a
\emph{nested} size ladder (1k--100k, each size a prefix of the next and
extensible to larger sizes without re-generation) in six content
configs---code-only JSONL plus Parquet with binary STEP, STL, and eight-view GPU
renders---and is browsable through a committed in-browser gallery, and any corpus
can be regenerated from a versioned seed list. Generation sustains \mbox{$\sim$10} validated parts/s on a 24-core
...
\rowcolor{panel}\textbf{CadQuarry} & $\geq$100k\textsuperscript{c} & \textbf{parametric CadQuery} $+$ B-rep, STL, renders & procedural & \yes & \textbf{\cc{}}\\
\bottomrule
\end{tabularx}
...
attribution. \textsuperscript{c}\,Published as a nested ladder topping at 100k;
the generator is unbounded, the base extends to larger sizes without
re-generation, and any size is reproducible from the seed.}
\label{tab:compare}
\end{table}
...
On a \textbf{200k} generation, \textbf{all 200{,}000} accepted parts have
...
\textbf{Reproducibility.} A versioned seed list pins the published ladder to a
single base seed, a \code{config}, and a generator version; each size tag is a
prefix length of that one base corpus. So the published Parquet/JSONL is a
\emph{convenience artifact}: the generator plus the seed list is the canonical
source, and any size can be regenerated (or the base extended) rather than
...
\textbf{Multiple output passes.} The geometry pass' G-buffer is reused to emit
four \emph{legibility-first} image variants per view at almost no extra cost, so
each viewpoint yields a small multi-modal stack rather than a single picture:
the \textbf{shaded} thumbnail above; a \textbf{normal} map (view-space normals
encoded as RGB); a \textbf{depth} map (linearized view depth, near bright); and
an \textbf{edge} overlay (creases, silhouette and depth steps as black lines on
a transparent background, i.e.\ a technical-drawing layer, with a tunable
crispness so noisy real scans can be softened). The shaded pass is the default
and keeps the bare \code{\{view\}.png} name; the others are written as
\code{\{view\}\_\{pass\}.png} siblings and, when present, surface as extra
\code{render\_\{view\}\_\{normal,depth,edge\}} image columns.
...
\code{render\_*}   & The eight fixed-viewpoint GPU renders---shaded by default, with optional normal / depth / edge passes per view (\cref{sec:render}).\\
\addlinespace
...
\end{axis}
\end{tikzpicture}
\caption{Realized family distribution on a 200k generation vs.\ the configured
...
The corpus is published on the Hugging Face Hub~\cite{huggingfacedatasets} as a
\emph{nested} size ladder---\code{1k}, \code{2k}, \code{5k}, \code{10k},
\code{20k}, \code{50k}, \code{100k}---in which each size is a prefix of the next.
Generation is a deterministic prefix stream (each attempt is a pure function of
the seed and its index, and the accept/dedup decision runs in strict attempt
order), so the first $N$ accepted parts of a run \emph{are} the size-$N$ corpus.
Every tag is therefore a zero-cost \code{head -N} slice of one base corpus
(shared seed): the base is generated, validated, exported, and rendered once,
and it can be \emph{extended} to larger sizes later by replaying the attempt
stream from a recorded high-water mark and appending only the new parts---reusing
the existing parts and geometry untouched, with no re-generation.
Each size comes in six content configs (\cref{tab:configs}) so a consumer fetches
only what they need, from a few-megabyte code-only pull to full geometry.
Code-only data is JSONL with the parametric \code{source} and \code{params}
inlined; geometry configs are Snappy-compressed Parquet with typed binary columns
(\code{render\_*} images, \code{stl\_bytes}, \code{step\_bytes}). Each config is
also published restricted to tiers~0--2 (a \code{-t0-2} suffix) for consumers who
want to exclude the most complex parts.
...
\textbf{One-command pipeline.} \code{build} generates and exports the single base
corpus the ladder slices from (and \code{build~-{}-extend-to~N} grows it in place,
reusing prior work); \code{publish} packs each size as a prefix slice and uploads
them as Hugging Face configs; \code{build\_sample} refreshes the committed
preview. Builds are resumable and secrets (the HF token) are read from the
environment and never logged or committed.
...
does not cover free-form/organic surfaces; solver-built families are
